...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $result = $recetteController->create($_POST, $ingredients);

    if ($result['success']) {
        header('Location: listRecette.php?success=added');
        exit;
    }
...
<style>
  ...
  .form-card-header{background:linear-gradient(135deg,var(--green-pale),#d4edbc);padding:1.5rem 2rem;border-bottom:1px solid var(--border);}
  .form-card-header h2{font-family:"Playfair Display",serif;font-size:1.3rem;color:var(--green-dark);}
  .form-card-header p{font-size:.83rem;color:var(--grey);margin-top:.2rem;}
  ...
  .btn-add-ing:hover{background:var(--green-pale);border-color:var(--green-main);border-style:solid;}
  ...
  .btn-save{background:linear-gradient(135deg,var(--green-main),var(--green-dark));color:var(--white);border:none;padding:.8rem 2rem;border-radius:12px;font-family:"DM Sans",sans-serif;font-size:.95rem;font-weight:700;cursor:pointer;flex:1;transition:transform .2s,box-shadow .2s;}
  .btn-save:hover{transform:translateY(-2px);box-shadow:0 8px 24px rgba(45,106,31,.35);}
  ...
  @media(max-width:768px){.content-section,.page-hero{padding:2rem 1.5rem;}.grid-2{grid-template-columns:1fr;}.ing-row{grid-template-columns:1fr auto;}.ing-qty{display:none;}}
</style>
  ...
  <div class="page-hero-inner">
    <div>
      <div class="breadcrumb">
        <a href="index.php">Accueil</a> › <a href="listRecette.php">Recettes</a> › Ajouter
      </div>
      <h1>➕ Créer une Recette</h1>
      <p>Composez une nouvelle recette avec ses ingrédients</p>
    </div>
  </div>
</div>
      ...
      <form method="POST" enctype="multipart/form-data" id="recetteForm" novalidate>
        ...
        <div class="form-group">
          ...
          <input type="text" name="nom" class="form-control <?= isset($errors['nom'])?'is-invalid':'' ?>"
            ...
            placeholder="ex : Bowl Quinoa & Avocat, Poulet au citron…" maxlength="150"/>
        ...
        <div class="form-group">
          ...
          <textarea name="description" class="form-control <?= isset($errors['description'])?'is-invalid':'' ?>"
            placeholder="Décrivez la recette, ses saveurs, ses bienfaits…"><?= htmlspecialchars($old['description'] ?? '') ?></textarea>
        ...
        <div class="grid-2">
          <div class="form-group">
            ...
            <input type="number" name="temps_preparation"
              ...
              value="<?= htmlspecialchars($old['temps_preparation'] ?? '') ?>"
              placeholder="ex : 25" min="1" max="1440"/>
          ...
          <div class="form-group">
            ...
            <select name="difficulte" class="form-control <?= isset($errors['difficulte'])?'is-invalid':'' ?>">
              ...
              <?php foreach (['facile'=>'😊 Facile','moyen'=>'🔥 Moyen','difficile'=>'💪 Difficile'] as $val => $label): ?>
                <option value="<?= $val ?>" <?= ($old['difficulte'] ?? '') === $val ? 'selected' : '' ?>><?= $label ?></option>
              <?php endforeach; ?>
            </select>
        ...
        <div class="form-group">
          ...
          <div class="img-upload-zone" onclick="document.getElementById('imgInput').click()">
            ...
            <p><strong>Cliquez pour choisir</strong> ou glissez une image</p>
        ...
        <div class="ing-list" id="ingList">
          <?php
          $oldIngs = [];
          if (!empty($old['ing_aliment'])) {
              foreach ($old['ing_aliment'] as $idx => $aid) {
                  $oldIngs[] = ['aliment_id' => $aid, 'quantite' => $old['ing_quantite'][$idx] ?? ''];
              }
          }
          if (empty($oldIngs)) $oldIngs[] = ['aliment_id' => '', 'quantite' => ''];
          foreach ($oldIngs as $i => $ing): ?>
          <div class="ing-row" id="ing-<?= $i ?>">
            <select name="ing_aliment[]" class="ing-select" required>
        ...
        <div class="form-actions" style="margin-top:2rem;">
          ...
          <button type="submit" class="btn-save">💾 Créer la recette</button>
        </div>
      </form>
    </div>
  </div>
...
document.getElementById('recetteForm').addEventListener('submit', function(e) {
  ...
  if (!ingValid) {
    ingErr.classList.add('show');
    valid = false;
  } else {
    ingErr.classList.remove('show');
  }

  if (!valid) {
    e.preventDefault();
    document.querySelector('.is-invalid, [style*="border-color: rgb(231"]')?.scrollIntoView({behavior:'smooth',block:'center'});
  }
});
</script>
