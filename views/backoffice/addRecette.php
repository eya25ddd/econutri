...
══════════════════════════════════════════ -->
    <div class="main-area">
      ...
      <div class="topbar">
        <div class="topbar-left">
          <div class="page-title">
            ...
            <span>Création d'une nouvelle recette EcoNutri</span>
          </div>
        </div>
      ...
      <div class="content">
        <div class="crud-header">
          <h1 class="crud-title">🍽️ Ajouter une Recette</h1>
            ...
            <div class="form-section">
              ...
              <div class="form-grid">
                <div class="form-group">
                  ...
                  <input type="text" id="nom" name="nom" class="form-input" value="<?php echo htmlspecialchars($old['nom'] ?? ''); ?>" required>
                </div>
                <div class="form-group">
                  <label class="form-label" for="categorie">Catégorie</label>
                  <select id="categorie" name="categorie" class="form-select">
                    <option value="plat" <?php echo ($old['categorie'] ?? '') === 'plat' ? 'selected' : ''; ?>>Plat principal</option>
                    <option value="entree" <?php echo ($old['categorie'] ?? '') === 'entree' ? 'selected' : ''; ?>>Entrée</option>
                    <option value="dessert" <?php echo ($old['categorie'] ?? '') === 'dessert' ? 'selected' : ''; ?>>Dessert</option>
                    <option value="boisson" <?php echo ($old['categorie'] ?? '') === 'boisson' ? 'selected' : ''; ?>>Boisson</option>
                  </select>
                </div>
                <div class="form-group">
                  <label class="form-label" for="temps_preparation">Temps de préparation (min) *</label>
                  <input type="number" id="temps_preparation" name="temps_preparation" class="form-input" value="<?php echo htmlspecialchars($old['temps_preparation'] ?? ''); ?>" required min="1">
                </div>
                <div class="form-group">
                  <label class="form-label" for="difficulte">Difficulté</label>
                  <select id="difficulte" name="difficulte" class="form-select">
                    <option value="facile" <?php echo ($old['difficulte'] ?? '') === 'facile' ? 'selected' : ''; ?>>Facile</option>
                    <option value="moyen" <?php echo ($old['difficulte'] ?? '') === 'moyen' ? 'selected' : ''; ?>>Moyen</option>
                    <option value="difficile" <?php echo ($old['difficulte'] ?? '') === 'difficile' ? 'selected' : ''; ?>>Difficile</option>
                  </select>
                </div>
              </div>
              <div class="form-group full-width">
                <label class="form-label" for="description">Description *</label>
                <textarea id="description" name="description" class="form-textarea" required><?php echo htmlspecialchars($old['description'] ?? ''); ?></textarea>
              </div>
            </div>
              ...
              <div id="ingredientsContainer">
                <div class="ingredient-row">
                  ...
                  <div class="form-group">
                    ...
                    <input type="number" name="ing_quantite[]" class="form-input" placeholder="100" min="0.1" step="0.1" required>
                  </div>
                  <button type="button" class="remove-ingredient-btn" onclick="removeIngredient(this)" style="display: none;">×</button>
                </div>
              </div>
    ...
    <script>
      ...
      // Show remove button for first row if there are multiple ingredients
      document.addEventListener('DOMContentLoaded', function() {
        const container = document.getElementById('ingredientsContainer');
        const rows = container.querySelectorAll('.ingredient-row');
        if (rows.length > 1) {
          rows.forEach(row => {
            row.querySelector('.remove-ingredient-btn').style.display = 'grid';
          });
        }
      });
    </script>
